...
export class ObsidianAPI {
    ...
    async getVaultInfo() {
        try {
            ...
            if (!response.ok) {
                throw ErrorFactory.obsidianConnectionFailed(new Error(`HTTP ${response.status}: ${response.statusText}`), { operation: 'getVaultInfo' });
            }
            ...
            throw ErrorFactory.obsidianConnectionFailed(error, { operation: 'getVaultInfo' });
        }
    }
    async listFiles(folder, recursive = false, includeMetadata = false) {
    ...
    async getDirectoryFileCount(folderPath) {
        try {
    ...
    async getFileContent(path) {
        ...
        try {
            ...
            if (!response.ok) {
                ...
                throw ErrorFactory.obsidianConnectionFailed(new Error(`HTTP ${response.status}: ${response.statusText}`), { operation: 'getFileContent', filePath: path });
            }
            ...
            throw new StructuredError(ErrorCode.FILE_ACCESS_DENIED, `Failed to get file content: ${error instanceof Error ? error.message : String(error)}`, { operation: 'getFileContent', filePath: path }, { cause: error });
        }
    }
    async getNote(path) {
        const normalizedPath = this.normalizePath(path);
    ...
    async resolveLinkPath(linkText) {
        try {
            ...
            const candidateFiles = allFiles.filter(file => {
                ...
                if (fileNameWithoutExt === linkWithoutExt)
                    return true;
                // Case-insensitive match
                if (fileName.toLowerCase() === linkText.toLowerCase())
                    return true;
                if (fileNameWithoutExt.toLowerCase() === linkWithoutExt.toLowerCase())
                    return true;
                return false;
            });
            if (candidateFiles.length > 0) {
                // Prefer exact matches, then case-sensitive, then case-insensitive
                const exactMatch = candidateFiles.find(f => f.path.split('/').pop() === linkText ||
    ...
    async searchFilesystemForBasename(dir, basename) {
        try {
    ...
    async searchFiles(filters) {
        try {
            ...
            if (filters.folders && filters.folders.length > 0) {
                filteredFiles = filteredFiles.filter(file => filters.folders.some(folder => file.path.startsWith(folder)));
            }
            // Apply file type filter
            if (filters.fileTypes && filters.fileTypes.length > 0) {
                filteredFiles = filteredFiles.filter(file => filters.fileTypes.some(type => file.path.endsWith(type)));
            }
    ...
    extractFrontmatterTags(content) {
        ...
        if (arrayFormatMatch) {
            ...
            const yamlTags = tagString.split(',').map(tag => tag.trim().replace(/^["']|["']$/g, '') // Remove surrounding quotes
    ...
    async patchContent(path, operation, targetType, target, content) {
        try {
            ...
            if (targetType === 'heading') {
                ...
                if (!targetFound) {
                    ...
                    throw new LoggedError(`Heading "${target}" not found in file "${path}". Available headings: ${availableHeadings || 'none'}`);
                }
                if (targetPath) {
