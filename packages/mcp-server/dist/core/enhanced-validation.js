...
export class InputValidator {
     ...
     */
    static validateInput(schema, input, toolName) {
        try {
            ...
            if (error instanceof z.ZodError) {
                ...
                throw ErrorFactory.validationFailed('input', input, issues.join('; '), { operation: toolName });
            }
            throw error;
        }
    }
     ...
     */
    static validateFilePath(path, context = 'file operation') {
        try {
            ...
            if (error instanceof z.ZodError) {
                throw ErrorFactory.validationFailed('path', path, error.issues[0]?.message || 'Invalid path', { operation: context });
            }
            throw error;
        }
    }
     ...
     */
    static validateFilePaths(paths, context = 'batch operation') {
        if (paths.length === 0) {
     ...
     */
    static validateRegexPattern(pattern, context = 'pattern matching') {
        try {
            ...
            if (error instanceof z.ZodError) {
                throw ErrorFactory.validationFailed('regex pattern', pattern, error.issues[0]?.message || 'Invalid regex pattern', { operation: context });
            }
            throw error;
        }
    }
     ...
     */
    static validateSearchQuery(query, allowRegex = false) {
        ...
        if (allowRegex) {
            try {
                ...
                throw ErrorFactory.validationFailed('query', query, 'Invalid regular expression', { operation: 'search' });
            }
        }
        return query.trim();
    }
     ...
     */
    static checkPathSecurity(path, context) {
     ...
     */
    static validateContent(content, maxSize = 10_000_000) {
...
export const validateSearchInput = (input) => InputValidator.validateInput(SearchInputSchema, input, 'search');
export const validateGetNotesInput = (input) => InputValidator.validateInput(GetNotesInputSchema, input, 'get_notes');
export const validateWriteNoteInput = (input) => InputValidator.validateInput(WriteNoteInputSchema, input, 'write_note');
export const validateBatchOperationInput = (input) => InputValidator.validateInput(BatchOperationInputSchema, input, 'batch');
export const validateFileOperationInput = (input) => InputValidator.validateInput(FileOperationInputSchema, input, 'files');
