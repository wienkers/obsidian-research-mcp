...
export class SectionOperationsManager {
    ...
    async getSectionContent(path, sectionIdentifier) {
    ...
    async updateSection(path, sectionIdentifier, newContent) {
        try {
    ...
    extractAllSections(lines, options) {
        const sections = [];
    ...
    finalizeSection(section, content, sections, options) {
    ...
    filterSections(allSections, identifiers, originalContent) {
        const filtered = [];
    ...
    findSectionByIdentifier(sections, identifier, originalContent) {
        if (typeof identifier === 'string') {
            // Simple string matching - look for heading that contains this text
            return sections.find(section => section.title.toLowerCase().includes(identifier.toLowerCase()) ||
                section.identifier.toLowerCase().includes(identifier.toLowerCase())) || null;
        }
        switch (identifier.type) {
            ...
            case 'pattern': {
                ...
                const pattern = new RegExp(String(identifier.value), 'i');
                return sections.find(section => pattern.test(section.title) || pattern.test(section.content)) || null;
            }
        }
        return null;
    }
    addSectionContext(sections, allSections, _options) {
    ...
    calculateSectionMetadata(content) {
        const wordCount = this.calculateWordCount(content);
        const lineCount = content.split('\n').length;
        const hasSubsections = /^#{2,6}\s+/.test(content);
        const contentTypes = [];
        if (/```[\s\S]*?```/.test(content))
            contentTypes.push('code');
        if (/^\s*[-*+]\s+/m.test(content) || /^\s*\d+\.\s+/m.test(content))
            contentTypes.push('list');
        if (/\|.*\|/.test(content))
            contentTypes.push('table');
        if (/\[\[.*?\]\]|\[.*?\]\(.*?\)/.test(content))
            contentTypes.push('link');
        if (/!\[\[.*?\]\]/.test(content))
            contentTypes.push('embed');
        if (contentTypes.length === 0 || content.trim().length > 0)
