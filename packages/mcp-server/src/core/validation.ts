 ...
 */
function preprocessToolArguments(toolName: string, input: any): any {
  ...
  const toolArrayParams: Record<string, string[]> = {
  ...
  function tryParseJsonArray(value: any): any {
 ...
 */
export function validateToolInput<T>(toolName: string, input: any): T {
  try {
    ...
    const schema = ValidationSchemas[schemaKey as keyof typeof ValidationSchemas];
    
    if (!schema) {
      throw new LoggedError(`No validation schema found for tool: ${toolName}`);
    }
    
    return schema.parse(preprocessedInput) as T;
  } catch (error) {
 ...
 */
function toolNameToSchemaKey(toolName: string): string {
 ...
 */
export function handleValidationError(error: Error, toolName: string, input: any): never {
