...
export interface SemanticChunk {
  ...
  context: {
    preceding: string;  // 512 chars
    following: string;  // 512 chars
...
export class ContentChunker {
  ...
  chunk(content: string, path: string, options: Partial<ChunkOptions> = {}): SemanticChunk[] {
    ...
    const chunks = this.createChunksWithContext(
      content, 
      semanticUnits, 
      headingHierarchy, 
      path, 
   ...
   */
  private identifySemanticUnits(content: string): Array<{
    type: SemanticChunk['boundaries']['semantic'];
    start: number;
    end: number;
    startLine: number;
    endLine: number;
        ...
        for (let j = i + 1; j < lines.length; j++) {
          ...
          if (nextHeadingMatch) {
            ...
            if (nextLevel <= currentLevel) {
              ...
              if (j > 0) sectionEnd += 1; // Add newline
              endLine = j - 1;
              break;
            }
          }
   ...
   */
  private buildHeadingHierarchy(content: string): HeadingPath[] {
    const hierarchy: HeadingPath[] = [];
    const lines = content.split('\n');
  ...
  private createChunksWithContext(
    ...
    for (let i = 0; i < semanticUnits.length; i++) {
      ...
      const relevantHeadings = headingHierarchy.filter(h => 
        h.line <= unit.startLine
      ).slice(-6); // Keep up to 6 levels of hierarchy
   ...
   */
  private postProcessChunks(chunks: SemanticChunk[], config: ChunkOptions): SemanticChunk[] {
    const processed: SemanticChunk[] = [];
   ...
   */
  private splitLargeChunk(chunk: SemanticChunk, config: ChunkOptions): SemanticChunk[] {
