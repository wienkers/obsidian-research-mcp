...
export interface ParsedLink {
  ...
  fullMatch: string;
  filePath: string;        // The actual file path/name part
  section?: string;        // Section heading (after #, but not ^)
  blockId?: string;        // Block ID (after #^)
  alias?: string;          // Display text (after |)
  isEmbed: boolean;        // Whether this is an embed (![[...]])
...
export class LinkUpdater {
  ...
  async updateLinks(options: LinkUpdateOptions): Promise<LinkUpdateResult> {
  ...
  private async updateBacklinks(
    ...
    for (const filePath of markdownFiles) {
      ...
      try {
        ...
        const linkMatches = this.findLinksToFile(
          content, 
          oldBasename, 
          options.oldPath, 
          newBasename,
  ...
  private async updateForwardLinks(
    options: LinkUpdateOptions,
    result: LinkUpdateResult
  ): Promise<void> {
   ...
   */
  private parseLink(linkContent: string, isEmbed: boolean = false, startIndex: number = 0): ParsedLink {
   ...
   */
  private parseMarkdownLink(linkText: string, linkTarget: string, startIndex: number = 0): ParsedLink {
   ...
   */
  private isLinkToTargetFile(parsedLink: ParsedLink, targetBasename: string, targetPath: string): boolean {
   ...
   */
  private createReplacement(parsedLink: ParsedLink, newBasename: string, newPath: string): string {
  ...
  private findRelativeLinks(content: string): LinkMatch[] {
    const matches: LinkMatch[] = [];
  ...
  private replaceLinks(content: string, matches: LinkMatch[]): string {
  ...
  private adjustRelativePath(relativePath: string, oldDir: string, newDir: string): string {
  ...
  private generateSummary(result: LinkUpdateResult): string {
  ...
  async previewLinkUpdates(options: LinkUpdateOptions): Promise<LinkUpdateResult> {
