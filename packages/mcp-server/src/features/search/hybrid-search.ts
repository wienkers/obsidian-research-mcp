import { 
  ...
  SearchResult, 
  SearchFilters 
} from '@obsidian-research-mcp/shared';
...
export class HybridSearchEngine {
  ...
  private async expandSearchOptimized(
      ...
      for (let i = 0; i < resultsToExpand.length && expansionCount < MAX_EXPANSION_NOTES; i += BATCH_SIZE) {
        ...
        const batchPromises = batch.map(async (result) => {
          try {
            ...
            let note: any = await cache.get(cacheKey);
            
            if (!note) {
              note = await Promise.race([
                obsidianAPI.getNote(result.path),
                new Promise<never>((_, reject) => 
                  setTimeout(() => reject(new Error('Note read timeout')), 5000)
                )
              ]);
            ...
            for (const linkedPath of linkedPaths) {
              ...
              try {
                const linkedNote = await Promise.race([
                  ...
                  new Promise<never>((_, reject) => 
                    setTimeout(() => reject(new Error('Linked note read timeout')), 3000)
                  )
                ]);
    ...
    if (currentDepth + 1 < depth && finalResults.length < 100) {
      const newResults = finalResults.filter(r => 
        r.metadata?.expansionDepth === currentDepth + 1
      ).slice(0, 5); // Only expand top 5 new results
  ...
  private async findBacklinksOptimized(targetPath: string): Promise<string[]> {
    return backlinkIndex.getBacklinks(targetPath);
  }

  private async performSemanticSearch(params: HybridSearchParams): Promise<SearchResult[]> {
    try {
  ...
  private async performStructuralSearch(params: HybridSearchParams): Promise<SearchResult[]> {
    try {
      ...
      let allFiles: any[];
      const hasStructuralFilters = params.structuralFilters && (
        params.structuralFilters.dateRange ||
      ...
      for (let i = 0; i < finalFiles.length; i += BATCH_SIZE) {
        ...
        const batchPromises = batch.map(file => 
          this.searchInFile(file, searchTerms, originalQuery)
        );
  ...
  private async getFilesWithTags(requiredTags: string[]): Promise<any[]> {
  ...
  private async filterFilesByTags(files: any[], requiredTags: string[]): Promise<any[]> {
      ...
      const batchPromises = batch.map(async (file) => {
        try {
          ...
          if (note.tags && note.tags.length > 0) {
            const hasMatchingTag = requiredTags.some(filterTag => 
  ...
  private combineResults(semantic: SearchResult[], structural: SearchResult[], originalQuery: string): SearchResult[] {
    ...
    for (const result of structural) {
      ...
      if (existing) {
        ...
        const titleHeaderBonus = this.calculateTitleHeaderBonus(
          result.path,
          result.content || '',
          originalSearchTerms,
          originalQuery
  ...
  private async applyFilters(results: SearchResult[], filters?: SearchFilters): Promise<SearchResult[]> {
    ...
    if (filters.hasProperty) {
      ...
      const propertyFiles = new Set<string>();
      let matchedFiles = 0;
      let checkedFiles = 0;
  ...
  private generateCacheKey(params: HybridSearchParams): string {
    const key = {
      query: params.semanticQuery,
  ...
  private calculateTextScore(content: string, searchTerms: string[], originalQuery: string, filePath: string): number {
    const contentLower = content.toLowerCase();
    let score = 0;
  ...
  private extractContextSnippets(content: string, searchTerms: string[]): string[] {
  ...
  private matchesProperties(frontmatter: Record<string, any>, properties: Record<string, any>): boolean {
   ...
   */
  private countUniqueMatches(queryWords: string[], titleWords: string[]): number {
   ...
   */
  private getUniqueWordsFromHeaders(content: string): string[] {
   ...
   */
  private checkWordOrder(queryWords: string[], titleWords: string[], matchedWords: string[]): boolean {
